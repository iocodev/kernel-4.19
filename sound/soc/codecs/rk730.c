			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	unsigned int sign = snd_soc_component_read32(component, RK730_DADC_SR_ACL);
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	struct soc_mixer_control *mc =
		(struct soc_mixer_control *)kcontrol->private_value;
	unsigned int val = snd_soc_component_read32(component, mc->reg);
	unsigned int sign = snd_soc_component_read32(component, RK730_DDAC_SR_LMT);
	unsigned int mask = (1 << fls(mc->max)) - 1;
	unsigned int shift = mc->shift;
	int mid = mc->max / 2;
	int uv;
...
static int rk730_probe(struct snd_soc_component *component)
{
	...
	ret = snd_soc_component_read32(component, RK730_HK_TOP_0);
	if (ret < 0) {
