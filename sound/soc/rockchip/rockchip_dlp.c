	      ...
	      dma_pointer_f dma_pointer)
{
	...
	a = dma_pointer(substream) % adrd->period_sz;
	...
	if (bsubstream->runtime && snd_pcm_running(bsubstream)) {
		...
		b = dma_pointer(bsubstream) % bdrd->period_sz;
	      ...
	      dma_pointer_f dma_pointer)
{
	...
	hw_ptr = dma_pointer(substream) % drd->period_sz;
		 ...
		 const struct snd_dlp_config *config)
{
	...
	ret = snd_soc_add_component(dev, &dlp->component, driver, NULL, 0);
