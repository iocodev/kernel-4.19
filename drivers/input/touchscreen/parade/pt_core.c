...
static char pt_ctp_version[64] = "0";
...
static void pt_stop_wd_timer(struct pt_core_data *cd)
{
	...
	pt_debug(cd->dev, DL_WARN, "%s: TTDL WD Stopped\n", __func__);
}
...
//2020-05-28 
static ssize_t pt_ctp_version_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int len;

	sprintf(buf, "%s", pt_ctp_version);
	printk("ctp version:%s\n",buf);
	len = strlen(buf);
	return len;
}

static DEVICE_ATTR(version, S_IRUGO|S_IWUSR|S_IWGRP, pt_ctp_version_show, 0);

static struct kobject *pt_ctp_kobj;
static int ctp_sysfs_init(void)
{
	int ret ;

	printk("%s: yitoa \n", __func__);
	pt_ctp_kobj = kobject_create_and_add("ctp", NULL);
	if (pt_ctp_kobj == NULL) {
		printk("%s: yitoa subsystem_register failed\n", __func__);
		ret = -ENOMEM;
		return ret;
	}
	ret = sysfs_create_file(pt_ctp_kobj, &dev_attr_version.attr);
	if (ret) {
		printk("%s: sysfs_create_file version failed\n", __func__);
		return ret;
	}
	return 0 ;
}

static void ctp_sysfs_deinit(void)
{
	sysfs_remove_file(pt_ctp_kobj, &dev_attr_version.attr);
	kobject_del(pt_ctp_kobj);
}
...
static void pt_release_modules(struct pt_core_data *cd)
{
	...
	mutex_unlock(&module_list_lock);
	pt_debug(cd->dev, DL_WARN, "%s: release_modules finished.\n", __func__);
}
...
error_after_startup:
	...
	pr_err("%s PARADE error_after_startup #### 0\n", __func__);

error_setup_irq:
error_detect:
	pr_err("%s PARADE error_setup_irq error_detect #### 1\n", __func__);
	if (cd->cpdata->init)
	...
	pr_err("%s PARADE error_setup_irq error_detect #### 2\n", __func__);
	kfree(cd);
error_alloc_data:
error_no_pdata:
	pr_err("%s failed.\n", __func__);
