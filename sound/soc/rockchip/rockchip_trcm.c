...
static int dmaengine_trcm_hw_params(struct snd_pcm_substream *substream,
...
dmaengine_pcm_set_runtime_hwparams(struct snd_pcm_substream *substream)
{
	...
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, SND_DMAENGINE_TRCM_DRV_NAME);
...
static int dmaengine_trcm_open(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, SND_DMAENGINE_TRCM_DRV_NAME);
...
static int dmaengine_trcm_close(struct snd_pcm_substream *substream)
{
	struct dmaengine_trcm_runtime_data *prtd = substream_to_prtd(substream);
...
static snd_pcm_uframes_t dmaengine_trcm_pointer(struct snd_pcm_substream *substream)
{
	struct dmaengine_trcm_runtime_data *prtd = substream_to_prtd(substream);
...
static int dmaengine_trcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, SND_DMAENGINE_TRCM_DRV_NAME);
...
static int dmaengine_trcm_dma_guard_new(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, SND_DMAENGINE_TRCM_DRV_NAME);
	struct dmaengine_trcm *trcm = soc_component_to_trcm(component);
...
static int dmaengine_trcm_new(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, SND_DMAENGINE_TRCM_DRV_NAME);
	struct dmaengine_trcm *trcm = soc_component_to_trcm(component);
	...
	ret = dmaengine_trcm_dma_guard_new(rtd);
	if (ret)
		return ret;
...
static const struct snd_pcm_ops dmaengine_trcm_ops = {
...
static const struct snd_soc_component_driver dmaengine_trcm_component = {
	...
	.ops		= &dmaengine_trcm_ops,
	.pcm_new	= dmaengine_trcm_new,
};
...
static int snd_dmaengine_trcm_register(struct device *dev)
{
	...
	ret = snd_soc_add_component(dev, &trcm->component, driver, NULL, 0);
	if (ret)
		goto err_free_dma;
