...
*/
    ...
    ret = GTP_GPIO_REQUEST(ts->pwr_pin, "GTP_PWR_PORT");
    if (ret < 0)
    {
        GTP_ERROR("1Failed to request GPIO:%d, ERRNO:%d",(s32)ts->pwr_pin, ret);
        GTP_GPIO_FREE(ts->pwr_pin);
        return -ENODEV;
    }
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (of_property_read_u32(np, "tp-size", &val)) {
    	dev_err(&client->dev, "2 no max-x defined\n");
    	return -EINVAL;
    }
    ...
    if (of_property_read_u32(np, "max-x", &val)) {
    	dev_err(&client->dev, "1 no max-x defined\n");
    	return -EINVAL;
    }
    //ts->abs_x_max = val;
    if (of_property_read_u32(np, "max-y", &val)) {
    	dev_err(&client->dev, "1 no max-y defined\n");
    	return -EINVAL;
    }
    //ts->abs_y_max = val;
