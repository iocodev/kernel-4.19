		...
		struct device_node *core_node)
{
	...
	pr_debug("%s: hid_desc_reg:%d level_irq_udelay:%d vendor_id:%d prod_id:%d\n"
...
int pt_devtree_create_and_get_pdata(struct device *adap_dev)
{
	...
	for_each_child_of_node(adap_dev->of_node, core_node) {
		...
		irq_gpio = of_get_named_gpio(adap_dev->of_node, "parade,irq-gpios", 0);
		if (gpio_is_valid(irq_gpio)) {
			pdata->core_pdata->irq_gpio = irq_gpio;
		} else {
			pr_err("%s: failed to get irq gpio\n", __func__);
			break;
		}

		rst_gpio = of_get_named_gpio(adap_dev->of_node, "parade,rst-gpios", 0);
